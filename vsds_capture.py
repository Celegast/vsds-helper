...
The tool will:
  ...
  6. Append one complete row to output/vsds_scans.csv
...
def main():
    ...
    def on_capture():
        ...
        try:
            ...
            # 4. Also save scrollbar sample (true_total now known)
            sample_row = {
                'screenshot':    prefix,
                'true_total':    result['total_count'],
                'visible_count': result['visible_count'],
            ...
            if result['max_distance_ly'] is not None:
                print(f"  Max dist: {result['max_distance_ly']} ly")
